...
- **Keys:** `remote_key_inject{key_code, direction}`; keycode names/numbers are the `RemoteKeyCode` enum in `remotemessage.proto`. `direction=SHORT` works for navigation keys, but **Chromecast ignores SHORT for the volume group** (VOLUME_UP/DOWN/MUTE) — those must be sent as a press/release pair: `START_LONG` then `END_LONG` (this is what the Google TV app sends, per the v2 protocol description). The same pair on any key gives press-and-hold auto-repeat. `POST /api/key` takes an optional `direction` accordingly; the web UI uses pointerdown/up for d-pad and volume.
