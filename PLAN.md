...
**Project complete (all 8 phases + extras: app-link buttons, PWA manifest/icons, adaptive status polling, README, in-browser log viewer, Samsung TV volume fallback, semantic-release versioning). Next potential steps, in rough priority order:**
...
- **Samsung TV volume fallback (2026-07-20, verified live on a Samsung QE55Q60TAUXXC):** since volume genuinely cannot be controlled via the Android TV Remote protocol on this Chromecast (`volume_max == 0`, confirmed against the reference Python lib), `main/samsung_tv.c` adds a second, independent WebSocket connection straight to the Samsung TV's own local remote API and reroutes `VOLUME_UP`/`VOLUME_DOWN`/`VOLUME_MUTE` there when connected. Protocol details verified against `xchwarze/samsung-tv-ws-api` and recorded in `CLAUDE.md`. New managed dependency `espressif/esp_websocket_client`; new Kconfig `ATV_SAMSUNG_TV_IP` (empty = feature disabled). Pairing token persisted in NVS via `store_get/set_samsung_token`. **Hardware surprise:** plain `ws://:8001` (the path the reference docs made sound simplest) got an instant `ms.channel.unauthorized` with no on-screen prompt on this TV's 2020 firmware; only `wss://:8002` actually triggers the prompt — required enabling `CONFIG_ESP_TLS_INSECURE`/`CONFIG_ESP_TLS_SKIP_SERVER_CERT_VERIFY` (now in `sdkconfig.defaults`) since esp-tls refuses an unverified TLS connection unless that's explicit. Confirmed: pairing prompt approved on TV, token persisted across reboot, volume/mute buttons work from the web UI.
- **Versioning + release automation (2026-07-20):** `main/version.h` (`FW_VERSION`) is the single source of truth, logged at boot and exposed via `/api/status` + the web UI footer. Tagged `v0.1.0` by hand as the first release. Then automated it: PR titles must be Conventional Commits (`.github/workflows/pr-title.yml`, `amannn/action-semantic-pull-request`), and every push to `main` runs semantic-release (`.github/workflows/release.yml`, `.releaserc.json`) — it bumps `FW_VERSION` via `tools/bump_version.sh`, updates `CHANGELOG.md`, commits, tags, and publishes a GitHub Release. `package.json`/`package-lock.json` exist only for this CI tooling, not the firmware itself.
